...
echo $result
aws inspector list-findings --region ${region} --assessment-run-arns "${result}" --filter "severities=High"
cnt_inform=$(aws inspector list-findings --region ${region} --assessment-run-arns "${result}" --filter "severities=Informational"| jq -r .findingArns[]|wc -l|tr -d ' ')
cnt_high=$(aws inspector list-findings --region ${region} --assessment-run-arns "${result}" --filter "severities=High"| jq -r .findingArns[]|wc -l|tr -d ' ')
